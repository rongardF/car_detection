...
class BlobStorageClient(AbstractBlobStorageClient):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self._session = aioboto3.Session(
            aws_access_key_id=config.require_config("SCALEWAY_ACCESS_KEY"),
            aws_secret_access_key=config.require_config("SCALEWAY_SECRET_KEY"),
            region_name="pl-waw",
        )
    ...
    async def upload(self, blob: BlobFileContainerDto) -> None:
        async with self._session.client(
            "s3",
            endpoint_url=self._config.require_config("SCALEWAY_BLOB_ENDPOINT"),
            config=BotoConfig(signature_version="s3v4"),
        ) as s3:
            try:
                await s3.put_object(
                    Bucket=self._config.require_config("SCALEWAY_BUCKET"),
                    Key=blob.blob_path,
                    Body=blob.data
                )
                return True
